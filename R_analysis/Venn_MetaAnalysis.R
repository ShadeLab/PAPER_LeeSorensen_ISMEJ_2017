...
dev.off()
setEPS()
postscript("SFig6B.eps", width = 4.000, height=4.000, pointsize=8,paper="special")
vennDiagram(v)
dev.off()
setwd("~/GitHub_Repos/ShadeLab/PAPER_LeeSorensen_inprep/R_analysis/")
...
class.m <- cbind(fa.m, rec.m, ref.m)
colnames(class.m) <- c("FireAffected", "Recovered", "Reference")
colSums(class.m)
...
dev.off()
setEPS()
postscript("SFig6A.eps", width = 4.000, height=4.000, pointsize=8,paper="special")
vennDiagram(v)
dev.off()